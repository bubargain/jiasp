...
window.hzmr.push("topicNavigation:1595");
		...
		if (!view) {
			view = $("<div id='addToIdeabookDialog'></div>")
				...
				.append(
					...
					+ "</table>"
					+ "<div id='postToFacebookDiv'><input id='postToFacebook' name='postToFacebook' type='checkbox' /><img src='http://www.houzz.com/res/1595/pic/spacer.gif?v=1595' />"
...
window.hzmr.push("spaceActions:1595");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}

/*************  End spaceActions.js  **************/
/************* Start jquery-ui-1.8.17.custom.min.js **************/
try {// <script>
/*!
 * jQuery UI 1.8.17
 *
...
$.widget( "ui.combobox", {
	...
	_create: function() {
		...
		var input = this.input = $( "<input class='ui-combobox'>" )
			.val( value )
			.attr('id','ui-comboboxInput-'+select.attr('id'))
			.attr('placeholder',self.options.placeholderText)
			.insertAfter( select )
			.autocomplete({
				delay: 0,
				minLength: 0,
				...
				source: function( request, response ) {
					...
					response( select.children( "option" ).map(function() {
						...
						if ( this.value  && ( !request.term || matcher.test(text) ) )
							if (!request.term || !($(this).hasClass(self.options.excludedSourceCls))) {
								return {
									label: text.replace(
										new RegExp(
											"(?![^&;]+;)(?!<[^<>]*)(" +
				...
				change: function( event, ui ) {
					if ( !ui.item ) {
						...
						if ( !valid ) {
							...
							var currentText = select.find('option:selected').html();
								$( this ).val( currentText );
							//select.val( "" );
							input.data( "autocomplete" ).term = currentText;
							return false;
						}
					}
				}
		...
		this.input.closest('form').submit(function(){
			var comboboxes = $(this).find('.ui-combobox');
			comboboxes.trigger('change');
		});
		
//		var inputBorderDiv = $('<div class="ui-comboboxInputDiv">')
//				.append(input);
//		
//		comboContainer.append(inputBorderDiv);
		...
		this.button = $( "<div class='ui-combobox'><span class='ui-button-icon-primary ui-icon ui-icon-triangle-1-s'>&nbsp;</span></div>" )
			.attr( "tabIndex", -1 )
			.attr( "title", "Show All Items" )
			.attr( "id", 'ui-comboboxBtn-'+select.attr('id'))
//			.insertAfter( inputBorderDiv )
			.insertAfter(input)
			.removeClass( "ui-corner-all" )
			.addClass( "ui-corner-right ui-button-icon ui-button ui-widget ui-state-default ui-button-icon-only" )
			.click(function() {
 ...
 *
 * Copyright 2010, Scott Gonz谩lez (http://scottgonzalez.com)
 * Dual licensed under the MIT or GPL Version 2 licenses.
 *
...
window.hzmr.push("jquery-ui-1.8.17.custom.min:1595");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}

/*************  End jquery-ui-1.8.17.custom.min.js  **************/
/************* Start browsePhotos.js **************/
try {// <script>
HZ.ns('HZ.browsePhotos');
HZ.browsePhotos = {
	init : function(config) {
		// soften the image loading process
		/*
		var photos = $('img.space');
		if(!UIHelper.isIE7 && photos.length > 0) {
			if(!photos[0].complete) {
				photos.css("visibility", "hidden").load(function () {  
					$(this).css({"opacity":"0.8", "visibility":"visible"}).animate({'opacity': 1}, 500);  
				});
			}
		}*/

		// handling long description text in XL mode
	   	if(config.numImages < 2) {
			$(".whiteCard").each(function () {
				var card = $(this);
				var d = $(".photoDescription", card);
				var image = $(".imageWrapper", card);
				if(d.length > 0){
					var p = d.parent();
					var newHeight = parseInt(image.height()- (d.offset().top - p.offset().top) -30);
					if(newHeight < 0) newHeight = 50;
					d.height(newHeight);
					$(d).peekable({'moreHtml': "<a class='more'>...</a>", 'lessHtml':''});
				}
			});
			return;
		} 
		...
		var self = this,
		    offsetStart = 0,
		    index = 0;
		self.elements = $('.'+ config.selector);
		self.offset = config.numImages;
		
		while(index++ <= self.elements.size()/self.offset){
			(self.elements.slice(offsetStart, offsetStart+self.offset)).equalizeHeights();
			offsetStart = offsetStart + self.offset;	
		}
		return true;
	}
};

window.hzmr.push("browsePhotos:1595");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}
	...
	function requireInitialization() {
		if (!isInitialized) {
			FB.init({
				...
				cookie     : true, 
				xfbml      : true,
				channelUrl : 'http://www.houzz.com/res/1595/pic/channel.html?v=1595'
			});
...
HZ.publish.Manager = new function() {
	...
	this.processAction = function(_callback, _saveSetting) {
		...
		if (isUiShown && isActionEnabled) {
			...
			// determine if FB auth is required
			if (userPref == 1) {
				if (HZ.sns.JsSdk.isFbTokenExpired() && HZ.sns.JsSdk.isFbLoggedIn()) {
					isLoginAttempted = true;
					HZ.sns.JsSdk.requestFbAuth("email,publish_actions", fbAuthCallback, fbAuthCallback, false);
					return;
				} 
			}
		}
...
window.hzmr.push("publishManager:1595");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}
