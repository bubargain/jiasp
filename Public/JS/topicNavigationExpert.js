...
window.hzmr.push("topicNavigation:1593");
	...
	function requireInitialization() {
		if (!isInitialized) {
			FB.init({
				...
				cookie     : true, 
				xfbml      : true,
				channelUrl : 'http://www.houzz.com/res/1593/pic/channel.html?v=1593'
			});
...
HZ.auth.Manager = new function() {
    ...
    function newUserWelcomeDialog() {
        var dialogWidth, dialogHeight, welcomeLayout;
    	...
    	var params = { u : redirectUrl, d : "dialog", nuwl : welcomeLayout };
        if (signupSource)
            params.t = signupSource;
        if (sourcePage)
            params.s = sourcePage;
        ...
        if (displayMode == "web") {
        	...
        	HZ.auth.LoginWrapperDialog.show(url, function() { HZ.auth.Manager.loginComplete(); }, dialogWidth, dialogHeight);
        }
		...
		HZ.ajaz.Services.fbSignUp(function(response) {
			if (response && response.success) {
				...
				if (!hideLoadingDialog) {
					...
					HZ.ui.yamdi.Common.alert("We're sorry, please refresh and try again!", "We're sorry, but we encountered an unexpected issue - likely this page has timed out due to inactivity.  Please refresh this page and try again.<br/><br/>If you continue to have trouble logging in please contact support@houzz.com for assistance.");
				}
			}
		}, 
		false,
    ...
    this.fbLogin = function(options) {
    	...
    	HZ.sns.JsSdk.requestFbAuth("email,publish_actions", handleFbAuthSuccess, handleFbAuthFailure, false, params);
    };
...
window.hzmr.push("siteAuth:1593");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}
...
window.hzmr.push("jquery-ui-1.8.17.custom.min:1593");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}

/*************  End jquery-ui-1.8.17.custom.min.js  **************/
/************* Start socialShareBar.js **************/
try {//<script>
//Share buttons
...
var ShareBar = {
	MIN_WIN_WIDTH: 1153,
	PARKED_TRIGGER: 44, //mainArea y position + 15px margin
	FLOAT_TOP: 45, 
	PARKED_TOP: 15, 
	FLOAT_BAR_WIDTH: 80,
	FORCE_LEFT: false,
	...
	init:function() {
		...
		if ($('.signUpPromoContainer').length > 0) {
			this.FLOAT_TOP += 80;
		}
		(function() {
...
//				this.shareTable.style.display="block";
	...
	show:function() {
		if(!this.shareBar)
		{
			//if no floating shareBar, always show the inline one.
			if(this.shareTable)
			{
				this.shareTable.style.display="block";
			}
	...
	forceLeft: function () {
		var mainX = $('#mainContent').offset().left;
		
		$(this.shareBar).css('left', mainX-this.FLOAT_BAR_WIDTH);
	},
	reposition:function() {
		if(this.shareBar)
		{
			var winscroll = UIHelper.getScrollXY();
			if(winscroll[1] > this.PARKED_TRIGGER){
				this.shareBar.className = 'float';
				this.shareBar.style.top = this.FLOAT_TOP+"px";
			}
			else {
				this.shareBar.className = 'parked';
				this.shareBar.style.top = this.PARKED_TOP+"px";
			}
...
window.hzmr.push("socialShareBar:1593");
} catch (err) {HZ.utils.Logger.sendJsExceptionStackTrace(err)}

/*************  End socialShareBar.js  **************/