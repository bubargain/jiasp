$(function(){
	...
	// add by lxy 2011-03-29
	if (app == '')
	{
		var storeReg = /st[0-9]+\/[a-zA-z]+/i;// 店铺的静态地址规则 
		var goodsReg = /st[0-9]+\/[0-9]+/i;// 商品的静态地址规则 

		if (location.href.indexOf('my/distribution') != -1)
		{
			app = 'my_shipping';
		}
		else if (location.href.search(storeReg) != -1)
		{
			app = 'store';
		}
		else if (location.href.search(goodsReg) != -1)
		{
			app = 'goods';
		}
		else if (location.href.indexOf('cms_art_category') != -1)
		{
			app = 'cms_art_category';
		}
	}
...
$('span[ectype="inline_edit"]').click(function(){
    ...
    $('<input type="text">').css({border:'1px solid #ccc',width:'80%',height:'20px'})
                        ...
                        if(event.keyCode == 13)
                        {
                            ...
                            $.get(SITE_URL + '/index.php?c='+app+'&a=ajax_col&ajax=1',{id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                                if(data === 'false')
                                {
...
$('img[ectype="inline_edit"]').click(function(){
                ...
                $.get(SITE_URL + '/index.php?c='+app+'&a=ajax_col&ajax=1',{id:i_id,column:i_name,value:i_val},function(data){
                if(data === 'true')
                    {
    ...
    /*
	SITE_URL已经区分了前后台，这个地方就不用再分别处理了
