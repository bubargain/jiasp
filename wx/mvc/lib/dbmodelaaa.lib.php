...
GetName()																	//获取模型名称
CloseRs()																	//关闭记录集
QueryRow($strWhere,$fields='*',$order='',$table='')							//获取一行数据
QueryAll($strWhere,$fields='*',$order='',$limit='',$table='')				//获取所有数据
QueryRowToArray($strWhere,$fields='*',$order='',$table='')					//查询一行数据到数组
QueryAllToArray($strWhere,$fields='*',$order='',$limit='',$table='')		//获取所有数据到数组
QueryRowToArrayByName($strWhere,$fields='*',$order='',$table='')			//查询一行数据到数组并且数组键名是字段名
QueryAllToArrayByName($strWhere,$fields='*',$order='',$limit='',$table='')	//查询所有数据到数组并且键名是字段名
SqlAllToArray($sQuery)														//根据sql语句查询所有数据到数组
SqlAllToArrayByName($sQuery)												//根据sql语句查询所有数据到数组并且键名是字段名
SqlRowToArray($sQuery)														//根据sql语句查询一行到数组
SqlRowToArrayByName($sQuery)												//根据sql语句查询一行到数组并且键名是字段名
AddByArray($data,$retid=true,$debug=false)									//添加一个数组中的数据到数据库中
UpdateByArray($data,$keyValue,$debug=false)									//更新一个数组中的数据到数据库中
Delete($condition='')														//删除
DeleteByKeyValue($KeyValue)													//根据键值做删除
Execute($sql)																//执行一条sql语句
rowNum($result)																//返回记录数
get($params)																//根据参数获取一行记录
getInfo($id)																//根据id获取信息
find($params = array())														//根据一定条件找出相关数据
findAll($params = array())													//关联查找多对多的记录
getRelatedData($relation_name, $ids, $find_param = array())					//获取1对多，多对多的关联数据
add($data, $compatible = false)												//添加一条记录
createRelation($relation_name, $id, $ids)									//添加多对多关联的中间表关联数据
updateRelation($relation_name, $id, $ids, $update_values)					//更新多对多关系中的关系数据
unlinkRelation($relation_name, $conditions, $ids = null)					//去除多对多的关联链接
_relationLink($action, $relation_name, $id, $ids, $update_values = array())	//对多对多关联表操作
...
getDroppedData()															//获取实时保存的已删除记录
getCount()																	//获取统计数
_saveDroppedData($sql)														//临时保存已删除的记录数据
_updateLastQueryCount($sql)													//更新查询统计数
_getConditions($conditions, $if_add_alias = false)							//获取条件句段
_getSetFields($data)														//获取设置字段
getRealFields($src_fields_list)												//获取查询时的字段列表
_getFieldTable($owner)														//解析字段所属
_getInsertInfo($data)														//获取插入的数据sql
_valid($data)																//验证数据合法性
_initFindParams($params)													//初始化查询参数
_joinModel(&$table, $join)													//按指定的方式LEFT JOIN指定关系的表
_getJoinString($relation_info)												//获取JOIN连接字符串
getRelation($relation_name = null)											//获取关系信息
getRelationByType($relation)												//获取指定关系类型的关联信息
assemble($relation_name, $assoc_data, $pri_data)							//组合数据
dataEnough($data)															//检查数据是否足够
getRequiredFields()															//获取必须的字段列表
getOne($sql)																//获取一个数据
getRow($sql)																//获取一行数据
getCol($sql)																//获取一列数据
getAll($sql)																//获取全部数据
getAllWithIndex($sql_statement, $index_key)									//获取全部数据按索引
Exists($KeyValue)															//是否存在
GetCount($strWhere)															//获取总数
GetOne($sQuery)																//获取单个数据
...
class dbModel
{
    public $db = null;							/* 所映射的主数据表 */
    public $table = '';							/* 所映射的数据库表 */
    public $prikey= '';							/* 主键 */
    public $alias = '';							/* 别名 */
    public $_name   = '';						/* 模型的名称 */
    public $_prefix = '';						/* 表前缀 */
    public $_autov = array();					/* 数据验证规则 */
    public $_last_query_count = -1;				/* 查询统计 */
    public $_dropped_data = array();			/* 临时保存已删除的数据 */
    public $_relation = array();				/* 关系(定义关系时，只有belongs_to以及has_and_belongs_to_many需要指定reverse反向关系)*/
	
	//构造函数
	function __construct($params, $db)
	{
		$this->dbModel($params, $db);
	}
	
	//构造函数
	function dbModel($params, $db)
	{
        $this->db =& $db;
	...
	//查询一行到数组，要看这个返回是不是数组了，并且返回的下标是字段名
	...
	{
		return $this->add($data,false,$retid,$debug);
	}
	
    //添加一条记录
    function add($data, $compatible = false, $retid = true, $debug = false)
    {
		...
		if($debug)
		{
			echoMsg("{$mode} INTO {$this->table}{$insert_info['fields']} VALUES{$insert_info['values']}");
		}
	...
	public function UpdateByArray($data,$keyValue,$debug=false)
	{
		...
		$sql.=" where " . $this->prikey . "='" . $keyValue . "'";
		if($debug)
		{
			echoMsg($sql,"sql");
		}
	...
	{
		$sql="delete from " . $this->table . " where " . $this->prikey . "='" . $KeyValue . "'";
		return mysql_query($sql);
	}
    ...
    {
        return $this->db->getAllWithIndex($sql_statement, $index_key);
    }
	
	//是否存在
	function Exists($KeyValue)
	{
		$sQuery = "select count(1) from " . $this->table . " where " . $this->prikey . "='" . $KeyValue . "'";
		$nRet=$this->db->getOne($sQuery);
		if($nRet>0)
		{
			return true;
		}
