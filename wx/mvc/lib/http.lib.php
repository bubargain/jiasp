...
******************************************************************************************/
    class http {
        ...
        function _get_response( $get_body = true ) {
			...
			//add for timeout by wanjl@yoka.com
			$timeout = 5;
			$is_timeout = false;
            ...
            //end add for timeout by wanjl@yoka.com

            while ($continue) {
                $header = '';

                // Read the Response Headers
                while ( (($line = fgets( $this->_socket, 40960 )) != HTTP_CRLF || $header == '') && !feof( $this->_socket ) ) { 
            ...
            if ( strtolower( $this->_response->get_header( 'Transfer-Encoding' ) ) != 'chunked' && !$this->_keep_alive ) {
                while ( !feof( $this->_socket ) && !$is_timeout ) {
                    $body .= fread( $this->_socket, 40960 ); 
					//add for timeout by wanjl@yoka.com
					$info = stream_get_meta_data($this->_socket);
					if($info['timed_out']) {
						$is_timeout = true;
                ...
                if ( $this->_response->get_header( 'Content-Length' ) != null && intval($this->_response->get_header( 'Content-Length' )) > 0) {
                    ...
                    if ( $this->_response->get_header( 'Transfer-Encoding' ) != null ) {
                        if ( strtolower( $this->_response->get_header( 'Transfer-Encoding' ) ) == 'chunked' ) {
                        	$chunk_size = (integer)hexdec(fgets($this->_socket, 40960));
							$block_size = 0;
							while(!feof($this->_socket)) {
								if($block_size<$chunk_size) {
									$body .= fgetc($this->_socket);
									$block_size++;
								}
								else {
									//echo fread($fp,2);
