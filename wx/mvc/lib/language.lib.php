<?php
defined('IN_CJCMS') or exit();

// LANGUAGE CONSTANTS 设置语言常量
...
class CJLanguage
{
	// Contains the storage handler object 存储处理程序的变量
	var $_storage;
	
	// Contains the compiler handler object	编译处理程序的变量
	var $_compiler;
  
	// Currently selected language	当前选择的语言
	var $_language;
  
	// Charset to use	使用的字符集
	var $_charset = "utf8";
  
	// Contains the ranges for new vars 包含新语言常量的取值范围
	var $_indices = array
	(
	...
	// Will assign self by ref to this variable       smarty中用到的变量
	var $_smarty_this_name = "cj_language";
  
	// Use temp file indexing? 使用临时文件的存储
	var $_indexing = TRUE;
	var $_indexing_post_load = FALSE;
	var $_indexing_vars_global;
	var $_indexing_vars_page;
  
	// Contains lang vars already sent to the javascript api  包含已经发送的javascript的api
