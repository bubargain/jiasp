...
class dbModel extends Object
{
    ...
    function __construct($params, $db, $conn_or_conntype=self::SLAVE)
    {
        $this->dbModel($params, $db, $conn_or_conntype=self::SLAVE);
    }
	
    function dbModel($params, $db, $conn_or_conntype=self::SLAVE)
    {
        $this->db =& $db;
		...
		if(!$db)
		{
			if ($conn_or_conntype == self::SLAVE) 
			{
				$this->db = dbMysqlManager::getMaster();
			}
			else if ($conn_or_conntype == self::MASTER)
			{
				$this->db = dbMysqlManager::getSlave();
			}
			else if (is_object($conn_or_conntype))
			{
				$this->db = $conn_or_conntype;
			}
			else
			{
     ...
     *  @return mixed
     */
    function add($data, $compatible = false, $debug=false)
    {
		...
		if($debug)
		{
			echoMsg("{$mode} INTO {$this->table}{$insert_info['fields']} VALUES{$insert_info['values']}");exit;
		}
