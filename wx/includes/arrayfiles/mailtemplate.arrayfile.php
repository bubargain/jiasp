...
class MailtemplateArrayfile extends BaseArrayfile
{
    ...
    function getOne($key)
    {
		...
		$details_data=array();
		if(file_exists($this->_mail_user_dir . $key . '.php'))
		{
			$details_data=include($this->_mail_user_dir . $key . '.php');
		}
		else if(file_exists($this->_mail_default_dir . $key . '.php'))
		{
			$details_data=include($this->_mail_default_dir . $key . '.php');
		}
